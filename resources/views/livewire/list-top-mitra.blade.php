<div>
    ...
    {{ $this -> form }}
    <section class="p-4 m-8 bg-white rounded-xl shadow dark:bg-gray-900">
        <h2 class="mb-4 text-xl font-semibold text-gray-800 dark:text-gray-100">Daftar Mitra Teladan</h2>
    <div class="overflow-hidden mt-4 rounded-lg border border-gray-200 shadow-lg dark:border-gray-700">
                ...
                @foreach ($this->mergedData as $item)
                    <tr class="transition-all duration-300 ease-in-out">
                        ...
                        <td class="p-4 text-center border-t border-b border-gray-200 dark:border-gray-700">
                            ...
                            <pre class="p-1 mb-1 text-xs text-gray-400 bg-gray-100 rounded dark:bg-gray-800">@json($item)</pre>
                            {{-- Always show Nilai button for valid Mitra Teladan --}}
                            <button
                                wire:click="$emit('openNilai2Modal', {{ $item['id'] ?? 'null' }})"
                                class="px-3 py-1 mb-1 text-xs font-medium text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400"
                                @if(empty($item['id'])) disabled @endif
                            >
                                Nilai
                            </button>
                            {{-- Existing Finalisasi/Accepted logic --}}
                            @if ($item['status'] !== 'final')
                                <button
                                    wire:click='openModal(@json($item))'
                                    class="px-4 py-2 text-sm font-medium text-white rounded-md shadow transition duration-150 bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
                                >
                                    Finalisasi
                                </button>
                            @else
...
<div x-data="{ open: @entangle('isOpen') }" x-show="open" x-cloak>
    <div class="flex fixed inset-0 z-50 justify-center items-center bg-gray-500 bg-opacity-75">
